...
export async function onRequestPost(context) {
  ...
  await env.DB.prepare(
    'INSERT INTO sellers (id, name, code, email, phone, created_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).bind(id, String(name).slice(0, 200), cleanCode, String(email || '').slice(0, 200), String(phone || '').slice(0, 50), new Date().toISOString()).run();
